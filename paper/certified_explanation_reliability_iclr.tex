...
%   [ ] recast companion self-citations (caraker2026limits, caraker2026dash)
%       in third person for review, per double-blind convention
...
assumption at all) are machine-checked in Lean~4 --- to our knowledge the first
explanation-reliability instrument whose end-to-end guarantee chain is formally
verified. Retrospectively, on
...
observed ensemble, always. (The underlying Lean statement is the variance form
$\hat\sigma^2/(\hat\sigma^2+\hat\mu^2)$ over an arbitrary nonempty finite ensemble,
with no positivity hypotheses at all.) It is strictly sharper than two-sided Chebyshev
...
either, the 100\% observed transfer of Section~\ref{sec:validation} is consistent with
--- indeed anticipated by --- the theorems, rather than a fortunate regularity. To our
knowledge this makes the certificate the first \emph{explanation-reliability} instrument
with an end-to-end formally verified chain from guarantee statement to deployment
behavior; formal verification in ML has otherwise targeted models and robustness
properties rather than empirical instruments
\citep{decker2024provably,noguer2025mathematical}.
% TODO(due-diligence): distribution-free per-query guarantees are conformal in
% spirit — run a prior-art sweep of formally verified conformal prediction before
% submission; if any exists, keep the novelty claim scoped to explanation
% instruments (as now worded) and cite it.
...
\item \textbf{The guarantee holds.} The uncapped Cantelli bound holds for all 10{,}724
  claims in-sample, as the theorem requires. Out of sample, the machine-checked
  Hoeffding link (full-ensemble flip plus Hoeffding slack) covers $99.4\%$ of claims
  against its $95\%$ design level, and the end-to-end composite (Cantelli bound plus
  Hoeffding slack, Theorem~\ref{thm:transfer-iid}) covers $100\%$. The bound is
  honestly conservative: mean slack ${\approx}0.50$, expected from Cantelli looseness
  plus the finite-$m$ Hoeffding term.
...
German credit data (OpenML-31), we trained 60 bootstrap-retrained gradient-boosting
models per seed and kept those within a strict $2\%$ test-accuracy window of the best
--- an explicit equal-performance Rashomon filter (5 seeds) --- computing each denied
applicant's principal (top-SHAP) reason per model. \textbf{On average $84\%$ of denied applicants (range
...
\item \textbf{Per-model-class, not model-invariant.} A claim stable for one model class
  ...
  agreement sets. More broadly, bootstrap and re-seeded ensembles are a \emph{proxy}
  for Rashomon-set sampling (the credit study instead filters to an explicit
  equal-accuracy window); every guarantee is relative to the ensemble-generating
...
\citet{krishna2022disagreement}, and \citet{laberge2023partial} extract the consensus
partial order of attributions across a Rashomon set. We add the per-claim guarantee
layer: not \emph{whether} models disagree, nor only \emph{what} they agree on, but
\emph{which individual claims} survive disagreement, with a proof.
...
The cost is $M$ model fits, embarrassingly parallel; at the benchmark scale
($M{=}60$ random forests, tabular data) this is seconds to minutes per dataset.
...
\texttt{\#print axioms} under continuous integration; the main repository carries
{\ClaimMainAxioms} domain axioms ({\ClaimTotalAxioms} program-wide across the three
repositories of the artifact), none on the results used here. The experiment scripts
