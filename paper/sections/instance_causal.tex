...
\texttt{causalSystem} is exactly the witness pair $(h, h')$ from
Proposition~\ref{prop:causal-rashomon}.
...
This is verified in Lean~4 as \texttt{causal\_instance\_impossibility} in
\texttt{CausalInstance.lean} (three axioms: \texttt{CausalConfig},
\texttt{CausalExplanation}, \texttt{CausalObservable}), and as the
self-contained \texttt{causal\_discovery\_impossibility} in
\texttt{CausalDiscovery.lean} (zero additional axioms beyond the
\texttt{CausalOrientationProblem} structure).
