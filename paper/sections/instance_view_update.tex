...
base relation?  \citet{bancilhon1981update} proved that when the view function
is not injective, there is no unique translation---a result discovered 45
years before our framework, making it one of the earliest instances of
the Explanation Impossibility in disguise.
...
distinct base rows).  This is precisely the classical result of
\citet{bancilhon1981update}: non-injective views cannot be uniquely
translated back.
...
Synthetic census disaggregation: for 50 states with 1--98 counties, the
KL-divergence between the true county distribution and Dirichlet-sampled
alternatives scales with the number of counties (Pearson $r = 0.39$,
$p = 0.005$).  States with 1 county have KL~$= 0$ exactly.

\paragraph{Historical note.}
The view update problem was identified by \citet{bancilhon1981update} in 1981,
making it one of the oldest instances of the Explanation Impossibility in the
literature.  The fact that it shares the same formal structure as gauge
invariance in physics, syntactic ambiguity in linguistics, and the phase
