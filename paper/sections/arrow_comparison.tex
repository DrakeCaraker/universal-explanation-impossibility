...
\section{The Structural Parallel Between Arrow's Impossibility and the Explanation Trilemma}
\label{sec:arrow-comparison}
...
An \textsc{ExplanationSystem}$(\Theta, H, Y, \mathrm{observe},
\mathrm{explain}, \mathrm{incompatible})$ consists of a configuration
space~$\Theta$, an explanation space~$H$, an observable space~$Y$, an
observation map $\mathrm{observe} : \Theta \to Y$, a native explanation
map $\mathrm{explain} : \Theta \to H$, and an irreflexive
incompatibility relation on~$H$. We say the system has the
\emph{Rashomon property} if there exist $\theta_1, \theta_2 \in \Theta$
with $\mathrm{observe}(\theta_1) = \mathrm{observe}(\theta_2)$ and
$\mathrm{incompatible}(\mathrm{explain}(\theta_1),
\mathrm{explain}(\theta_2))$. For any such system, no explanation
$E : \Theta \to H$ is simultaneously:
\begin{enumerate}[label=\textbf{E\arabic*.},leftmargin=2.5em]
  \item \textbf{Faithful.}\; $\neg\,\mathrm{incompatible}(E(\theta),
    \mathrm{explain}(\theta))$ for all~$\theta$.
  \item \textbf{Stable.}\; $\mathrm{observe}(\theta_1) =
    \mathrm{observe}(\theta_2) \implies E(\theta_1) = E(\theta_2)$.
  \item \textbf{Decisive.}\; $\mathrm{incompatible}(\mathrm{explain}(\theta), h)
    \implies \mathrm{incompatible}(E(\theta), h)$ for all~$\theta, h$.
\end{enumerate}
