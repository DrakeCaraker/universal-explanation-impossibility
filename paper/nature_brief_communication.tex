...
$p = 6.3 \times 10^{-9}$); the codon entropy dose-response across 30 species
shows a perfect monotonic relationship with degeneracy level (Kruskal--Wallis
$p = 10^{-17}$); syntactic parsers disagree significantly more on ambiguous
sentences ($p = 9 \times 10^{-4}$); causal discovery algorithms show
orientation agreement of only 0.33 at finite sample size ($p = 2.4 \times
10^{-5}$ across 10 random seeds per condition); phase retrieval
