...
\author{%
  ...
  Bryan Arnold\textsuperscript{1},
  David Rhoads\textsuperscript{1}
  \\[4pt]
  \textsuperscript{1}\textit{Independent Researchers}
  \\[2pt]
...
in the Lean~4 proof assistant (82~files, 377~theorems, 0~unproved goals),
...
\citep{demoura2021lean4} (82~files, 377~theorems, 0~unproved goals).
...
electron densities \citep{hauptman1985}.
%
...
Boltzmann entropy).  The codon entropy dose-response across 30 species shows
a perfect monotonic relationship with degeneracy level (Kruskal--Wallis
$p = 10^{-17}$); syntactic parsers disagree significantly more on ambiguous
sentences ($p = 9 \times 10^{-4}$); causal discovery algorithms converge
only with infinite samples; phase retrieval reconstructions diverge
1.5$\times$ more without positivity constraints; and census disaggregation
ambiguity scales with aggregation granularity ($p = 0.005$).  In each case,
...
The impossibility is constructive: it identifies the exact trade-off.
The resolution is to average over equivalent configurations, sacrificing
decisiveness to gain stability and faithfulness in expectation.  Remarkably,
practitioners in each domain have independently converged on this strategy.
In physics, one works with gauge-invariant observables---field strengths,
not potentials \citep{jackson1999}.
In statistical mechanics, the microcanonical ensemble assigns equal
probability to all microstates sharing a macrostate, rather than selecting
one.
In statistics, one reports a CPDAG (completed partially directed acyclic
graph)---the equivalence class of indistinguishable causal graphs---rather
than committing to a single DAG \citep{verma1991equivalence}.
In mathematics, the minimum-norm solution (the Moore--Penrose pseudoinverse)
is the unique solution closest to the origin, effectively averaging over
the solution space.
In biology, codon usage tables report frequencies across synonymous codons
rather than selecting one.
In crystallography, Patterson maps---the autocorrelation of the diffraction
data---extract the information that survives the loss of phase
\citep{hauptman1985}.
...
Our theorem proves that this orbit-averaging strategy is automatically
stable: any explanation that is invariant under the symmetry group relating
equivalent configurations is guaranteed to produce the same output for
...
Mathlib \citep{mathlib2020}.  The formalization comprises 82~files,
377~theorems and lemmas, 72~axioms (all domain-specific; the core theorem
...
The nine machine learning instances (feature attribution, attention maps,
counterfactual explanations, concept probes, causal discovery, model
selection, saliency maps, token citation, and mechanistic interpretability),
together with empirical validation experiments for seven of these, are
documented in the Supporting Information (the full 61-page monograph,
available on arXiv).
