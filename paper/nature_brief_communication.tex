...
in the Lean~4 proof assistant (\ClaimMainFiles~files, \ClaimMainTheorems~theorems, 0~unproved goals),
...
\citep{demoura2021lean4} (\ClaimMainFiles~files, \ClaimMainTheorems~theorems, 0~unproved goals).
...
faithfulness in expectation.  This orbit-averaging resolution
is \emph{Pareto-optimal} within equivalence classes (Lean-verified; global
optimality argued via classical invariant decision theory): among all stable
explanation maps, no alternative can achieve strictly higher pointwise
faithfulness on every equivalence class.
...
Pareto-optimal stable resolution.
...
Mathlib \citep{mathlib2020}.  The formalization comprises \ClaimMainFiles~files,
\ClaimMainTheorems~theorems and lemmas, \ClaimMainAxioms~\texttt{axiom} declarations
(gradient-boosting infrastructure; the core theorem and all eight derived instances
use zero), and 0~\texttt{sorry} (unproved goals).  Each of the eight
