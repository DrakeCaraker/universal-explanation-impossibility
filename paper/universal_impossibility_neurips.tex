...
faithful, stable, and decisive when
...
In each case, underspecification induces a \emph{Rashomon property} on explanation
targets, and the same four-line proof closes the impossibility.
Empirical experiments confirm severe instability: 60\% attention argmax flip rate, 23.5\%
...
The framework is mechanically verified in Lean~4 (67~files, 319~theorems, 0~\texttt{sorry}).
\end{abstract}
...
(6)~Lean~4 formalization: 67~files, 319~theorems, 60~axioms, 0~\texttt{sorry}.
...
$\Params$ is a parameter space (ground truths),
$\sH$ is a hypothesis space with attribution map $\theta: \sH \to \Params$,
$Y$ is an explanation target space,
$\mathsf{obs}: \sH \to Y$ assigns each hypothesis its ``honest'' explanation,
$\mathsf{exp}: \sH \to Y$ is the explanation procedure under evaluation, and
$\incomp \subseteq Y \times Y$ is an incompatibility relation.
\end{definition}

\begin{definition}[Faithful, Stable, Decisive]
\label{def:three}
An explanation map $\mathsf{exp}$ is \emph{faithful} if $\mathsf{exp}(h) = \mathsf{obs}(h)$ for all $h$;
\emph{stable} if $\mathsf{exp}(h) = \mathsf{exp}(h')$ whenever $\theta(h) = \theta(h')$;
and \emph{decisive} if $\mathsf{exp}(h)$ is resolved (contains no ties or undecided queries) for all $h$.
\end{definition}

\begin{definition}[Rashomon Property]
\label{def:rashomon}
$\mathcal{S}$ has the \emph{Rashomon property} if for
every pair $(y, y') \in \incomp$ with $y \neq y'$, there exist $h, h' \in \sH$ with
$\theta(h) = \theta(h')$ such that $\mathsf{obs}(h) = y$ and $\mathsf{obs}(h') = y'$.
\end{definition}

The Rashomon property formalizes underspecification at the explanation level: data do not uniquely determine the explanation, even in principle.

\begin{theorem}[Universal Explanation Impossibility]
\label{thm:universal}
If an explanation system $\mathcal{S}$ has the Rashomon property, then no explanation map $\mathsf{exp}$ can simultaneously be faithful, stable, and decisive.
\end{theorem}

\begin{proof}
By the Rashomon property, there exist incompatible $y \neq y'$
with $(y, y') \in \incomp$ and hypotheses $h, h'$ satisfying
$\theta(h) = \theta(h')$, $\mathsf{obs}(h) = y$, $\mathsf{obs}(h') = y'$.
Suppose $\mathsf{exp}$ is faithful, stable, and decisive.  By faithfulness, $\mathsf{exp}(h) = y$ and $\mathsf{exp}(h') = y'$.  By stability and $\theta(h) = \theta(h')$, $\mathsf{exp}(h) = \mathsf{exp}(h')$, so $y = y'$---contradiction.
\end{proof}

\begin{corollary}
\label{cor:decidable}
If the data-generating process $\theta$ is not identifiable---i.e., $\exists\, h, h'$ with $\theta(h) = \theta(h')$ yet $\mathsf{obs}(h) \neq \mathsf{obs}(h')$---then the Rashomon property holds whenever $\incomp$ has at least one incompatible pair.
\end{corollary}
...
\textbf{Empirical:} 10 DistilBERT models, 200 SST-2 sentences. 100\% prediction agreement, \textbf{60\% argmax flip rate}, Kendall $\tau = 0.30$.
...
\midrule
Files & 67 & \texttt{UniversalImpossibility/} \\
Theorems \& lemmas & 319 & Verified by \texttt{grep -c} \\
Axioms & 60 & Domain-specific; core uses 0 \\
...
10 DistilBERT models with weight perturbations ($\sigma \in \{0.01, 0.02\}$) on 200 SST-2 sentences.  100\% prediction agreement.  Argmax flip rate: 60.0\% [CI: 59--61\%].  Mean Kendall $\tau$: 0.30 [CI: 0.29--0.31].
...
\begin{table}[h]
\centering
\caption{Axiom inventory (60 total). The core impossibility uses zero axioms.}
\label{tab:axioms}
\small
\begin{tabular}{lrl}
\toprule
...
Practitioners know explanations are unstable---so what is new?  Four things.
