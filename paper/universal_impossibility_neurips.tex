...
This \emph{Explanation Impossibility} is a meta-theorem unifying nine apparently unrelated
impossibilities: (1)~feature attribution, (2)~attention maps, (3)~counterfactual explanations,
(4)~concept probes, (5)~causal discovery, (6)~model selection, (7)~saliency maps (GradCAM),
(8)~token citation explanations, and (9)~mechanistic interpretability.
...
The framework is mechanically verified in Lean~4 (100~files, 491~theorems, 25~axioms, 0~\texttt{sorry}).
\end{abstract}
...
We show there is \emph{one} structural cause.  We prove an impossibility theorem for explanation that we call the \emph{Explanation Impossibility}: no explanation of an underspecified system can simultaneously be faithful, stable, and decisive.  The theorem requires only the \emph{Rashomon property}---the existence of equivalent hypotheses that honestly report incompatible explanations---and the proof is four lines.
A practitioner who discovers SHAP instability cannot escape by switching to attention maps, counterfactual explanations, or concept probes --- the impossibility follows each method for the same structural reason.

\textbf{Contributions.}
(1)~A universal impossibility theorem (Theorem~\ref{thm:universal}) unifying nine impossibilities under one proof.
(2)~Nine instances: feature attribution, attention maps, counterfactuals, concept probes, causal discovery, model selection, saliency maps (GradCAM), token citation explanations, and mechanistic interpretability---each a special case.
(3)~Empirical illustration for seven instances confirming severe instability.
...
(6)~Lean~4 formalization: 100~files, 491~theorems, 25~axioms, 0~\texttt{sorry}.

\textbf{Related work} is discussed in Appendix~\ref{app:related}. This paper generalizes the attribution impossibility of \citet{bilodeau2024impossibility} from feature attribution to arbitrary explanation systems. The companion paper (arXiv:XXXX.XXXXX) provides quantitative depth for the attribution instance; this paper supplies the abstract framework, eight additional instances, and the ubiquity result. For causal discovery, the Rashomon property is derived from first principles (Markov equivalence).
...
The theorem is tight: dropping any one property makes the remaining two simultaneously achievable (Proposition~\ref{prop:tightness}).
...
its configuration, $E = \mathsf{explain}$ satisfies all three properties simultaneously.
For systems with non-injective observations and compatible-but-distinct explanations
on the same fiber, the relationship is more nuanced.
...
We instantiate Theorem~\ref{thm:universal} in nine domains.  For each, we specify the 6-tuple, verify the Rashomon property, and state the impossibility.  Proof sketches, empirical details, and figures are in Appendix~\ref{app:instances}.
...
\begin{table}[t]
\centering
\caption{Empirical illustration across four explanation types. All instability measures have 95\% bootstrap CIs excluding zero. These experiments demonstrate that the Rashomon property produces measurable instability in practice, not that the theorem depends on experimental evidence.}
...
\begin{table}[t]
\centering
\caption{$G$-invariant resolutions across nine domains.}
...
\DASH{} is the prototype: it averages SHAP values over retrained models, achieving consensus equity $\E[\varphi_j] = \E[\varphi_k]$ for symmetric features, and is Pareto-optimal among stable attribution methods for within-group pairs (Lean-verified; \texttt{dash\_within\_group\_dominance}), with global optimality argued via classical invariant decision theory. CPDAGs, ensemble concept probes, and robust recourse are analogous instances of the orbit-averaging resolution. The resolution strategy is structurally analogous to invariant decision procedures in the Hunt--Stein framework \citep{lehmann2005testing}.
...
The theorem and all nine instances are mechanically verified in Lean~4 \citep{demoura2021lean4} with Mathlib \citep{mathlib2020}.
...
\paragraph{Proof.}
By the Rashomon property, $\exists\, \theta_1, \theta_2$ with $f_{\theta_1} = f_{\theta_2}$ yet $\operatorname{argmax} \alpha_{\theta_1} \neq \operatorname{argmax} \alpha_{\theta_2}$.  Theorem~\ref{thm:universal} yields the impossibility.  Lean: \texttt{attention\_impossibility\_constructive} in \texttt{AttentionInstanceConstructive.lean}.
...
\paragraph{Proof.}
Models in $\mathcal{R}_\varepsilon$ place boundaries on opposite sides of query $x_0$, producing counterfactuals in opposite directions.  Theorem~\ref{thm:universal} applies.  Lean: \texttt{counterfactual\_impossibility\_constructive} in \texttt{CounterfactualInstanceConstructive.lean}.
...
\paragraph{Proof.}
Let $\theta_1, \theta_2$ witness the Rashomon property with $f_{\theta_1} = f_{\theta_2}$ and $(v_{\theta_1}, v_{\theta_2}) \in \incomp$.  Faithfulness forces $\mathsf{exp}(\theta_i) = v_{\theta_i}$; stability forces $v_{\theta_1} = v_{\theta_2}$---contradiction.  Lean: \texttt{concept\_impossibility\_constructive} in \texttt{ConceptInstanceConstructive.lean}.
...
\paragraph{Proof.}
For any CPDAG with an undirected edge $\{X,Y\}$, DAGs $h$ (with $X \to Y$) and $h'$ (with $Y \to X$) are Markov equivalent yet incompatible.  This is the classical result of \citet{verma1991equivalence} recast as a Rashomon property.  Lean: \texttt{causal\_impossibility\_constructive} in \texttt{CausalInstanceConstructive.lean}.
...
\paragraph{Proof.}
The $\varepsilon$-Rashomon set $\mathcal{R}_\varepsilon = \{h : \mathsf{obs}(h) \geq \mathsf{obs}(h^*) - \varepsilon\}$ generically contains models with incompatible rankings on different evaluation splits.  Lean: \texttt{model\_selection\_impossibility\_constructive} in \texttt{ModelSelectionInstanceConstructive.lean}.
...
\begin{table}[h]
\centering
\caption{Axiom inventory (83 total). The core impossibility uses zero axioms.}
\label{tab:axioms}
\small
\begin{tabular}{lrl}
\toprule
...
(1)~Nine domains share \emph{one} mechanism (the Rashomon property) rather than nine unrelated pathologies.
...
Each instance is individually known.  The contribution is the \emph{meta-theorem}: the same four-line proof closes all nine under a single structural condition.  Before this work, a practitioner encountering attention instability might switch to \SHAP{}, then counterfactuals, then concept probes---each time hoping to escape.  Theorem~\ref{thm:universal} shows no escape exists: the impossibility is structural, not method-specific.

\paragraph{AI disclosure.}
We used Claude (Anthropic) as an AI coding assistant for Lean~4 proof
development, experiment scripting, and manuscript drafting.  All
scientific claims and experimental designs were conceived by the human
authors.  All theorems are machine-checked.
