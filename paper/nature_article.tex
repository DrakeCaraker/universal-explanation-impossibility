...
10^{-9}$).  In biology, per-amino-acid codon usage entropy across 120
eukaryotic cytochrome~c sequences from NCBI RefSeq increases monotonically
with degeneracy level (Kruskal--Wallis $p = 2.0 \times 10^{-3}$ across
20~amino acids in 5 groups); methionine and tryptophan (degeneracy~1) have
entropy identically zero by the structure of the genetic code.  In
...
set ($p < 10^{-57}$).  In computer science, census disaggregation ambiguity using real 2020~Census county populations
scales with aggregation granularity (Spearman $\rho = 0.51$,
$p = 1.2 \times 10^{-4}$); the District of Columbia (one
...
differs across domains because the symmetry group differs
(Fig.~\ref{fig:structural})---orbit averaging for vector-valued
explanations, equivalence-class reporting for combinatorial ones---but the
abstract mathematical operation, projection onto the $G$-invariant
subspace, is identical in every case.
...
so the proof is valid for any system satisfying the hypothesis.  Seven of the
eight derived instances use Lean's decidable computation to constructively
...
identical observable output and incompatible explanations.  The causal
discovery instance uses a parallel proof structure within the symmetric
Bayes decision framework rather than instantiating the abstract
\texttt{ExplanationSystem}, but proves an equivalent impossibility with
