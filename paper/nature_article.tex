...
absent).  The $G$-invariant resolution---which sacrifices decisiveness
...
Second, the Rashomon property is the \emph{exact boundary}: when it is
absent---when each observation uniquely determines the internal
structure---all three properties are simultaneously achievable.  In this
case, the identity map $E(\theta) = \mathsf{exp}(\theta)$ is faithful,
stable (because equivalent configurations have identical internal
reports), and decisive.  This converse is a separate Lean-verified theorem,
confirming that the Rashomon property is both sufficient and necessary for
the impossibility.
...
set ($p < 10^{-57}$).  In computer science, census disaggregation ambiguity
scales with aggregation granularity ($p = 8.0 \times 10^{-37}$); the
District of Columbia (one county-equivalent) has KL divergence of exactly
zero.
