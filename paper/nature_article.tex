...
with identical scalar properties), protein folding degeneracy (multiple
sequences folding to the same structure), and quantum tomography
(multiple density matrices consistent with the same measurement
statistics) all exhibit the Rashomon property and should therefore satisfy
the impossibility.  In each case, the framework predicts that the
domain's practitioners will have independently converged on an
orbit-averaging resolution---a prediction that can be tested empirically.

The framework also suggests a broader research programme: for any domain
where explanation instability has been observed, one can ask whether the
Rashomon property holds, identify the symmetry group, and derive the
domain-specific optimal resolution.  The Lean formalization makes this
programme concrete: adding a new domain requires only defining an
...
automatically.  More broadly, the framework provides a universal
diagnostic: whenever a practitioner encounters explanation instability,
the first question should be whether the underlying system is
underspecified---and if so, the theorem guarantees that aggregation is
