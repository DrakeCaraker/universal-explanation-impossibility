...
steps, verified in the Lean~4 proof assistant (\ClaimMainFiles~files, \ClaimMainTheorems~theorems,
...
Crucially, this does not mean regulations are wrong to demand explainability.
It means the demand has a precise mathematical limit, and compliance should
be measured against the achievable optimum rather than an unrealizable ideal.

\section*{The explainability ceiling}

The Act uses the qualifier ``sufficiently'' (Article~13(1): ``sufficiently
transparent''), which creates room for a standard below the theoretical
maximum. The impossibility theorem identifies that ceiling; the question
is whether current regulatory expectations fall below it.
