...
Token citation & Citation flip & 34.5\% & 100\% \\
\bottomrule
\end{tabular}
\end{table}
...
The algebraic impossibility (Theorem~\ref{thm:universal}) has a natural
information-theoretic complement. Let $P$ be a probability measure over
$\Params$.

\begin{proposition}[Information-Theoretic Cost of Stability]
\label{prop:info-theoretic}
\emph{(Argued; paper proof.)}
If $E$ is stable, then $I(E;\,\Params \mid Y) = 0$ and
$I(E;\,\mathsf{explain}(\Params)) \leq I(Y;\,\mathsf{explain}(\Params))$.
When $H(\mathsf{explain}(\Params) \mid Y) > 0$ (the system is
underspecified), this bound is strictly less than
$H(\mathsf{explain}(\Params))$, so any stable $E$ forfeits at least
$H(\mathsf{explain}(\Params) \mid Y)$ bits of information about the native
explanation.
\end{proposition}

\begin{proof}[Proof sketch]
Stability means $E$ is a function of $Y$ alone: $E = g(Y)$ for some $g$.
Hence $I(E;\,\Params \mid Y) = 0$. By the data processing inequality
applied to the Markov chain $\mathsf{explain}(\Params) \to \Params \to Y
\to E$, we obtain $I(E;\,\mathsf{explain}(\Params)) \leq
I(Y;\,\mathsf{explain}(\Params))$. Decomposing:
$I(Y;\,\mathsf{explain}(\Params)) = H(\mathsf{explain}(\Params)) -
H(\mathsf{explain}(\Params) \mid Y)$. When $H(\mathsf{explain}(\Params)
\mid Y) > 0$, the bound is strictly less than
$H(\mathsf{explain}(\Params))$.
\end{proof}

The quantity $H(\mathsf{explain}(\Params) \mid Y)$ is the
\emph{fundamental information gap}: the minimum information that any
stable explanation must forfeit about the native explanation. This
formulation complements the algebraic impossibility
(Theorem~\ref{thm:universal}). Both identify the Rashomon property as the
root cause, but the information-theoretic version additionally quantifies
the minimum information loss. The Rashomon property holding with positive
probability is equivalent to $H(\mathsf{explain}(\Params) \mid Y) > 0$.
...
OCC SR~11-7~\cite{occ2011sr117} establishes three pillars for model
validation in banking that map directly onto the trilemma. Section~III.A
(\emph{Conceptual Soundness}) requires that ``the model's theoretical
construct, \ldots\ assumptions, and limitations should be assessed''---this
demands \emph{faithfulness}: the explanation must reflect the model's
actual structure and assumptions. Section~III.B (\emph{Outcomes Analysis})
requires ``comparing the model's outputs with the corresponding realized
outcomes'' across ``model versions''---this demands \emph{stability}: the
explanation must be consistent across model versions that produce
equivalent outcomes. Section~IV (\emph{Effective Challenge}) requires
``critical analysis by objective, informed parties'' yielding ``specific
findings''---this demands \emph{decisiveness}: the challenge must produce
specific, actionable findings, not hedged non-answers.
...
Effective challenge (Section~IV) is then directed at modeling choices---why
this Rashomon set? why this loss threshold?---rather than at explanation
artifacts that are mathematically underdetermined.

The same analysis applies to EU AI Act Article~13~\cite{euaiact2024}
(transparency) and NIST AI RMF MAP~1.5~\cite{nistai2023}
...
Table~\ref{tab:practitioner} provides actionable guidance. Each row
corresponds to a common practitioner goal and specifies which desideratum
is sacrificed, what method class achieves the remaining two, and example
tools.

\begin{table}[t]
\centering
\caption{Practitioner decision table: which desideratum to sacrifice for
each goal.}
...
\midrule
Debug THIS model & Stability & Model's own explanation & \SHAP{}, GradCAM \\
Monitor across retrains & Decisiveness & Aggregate over Rashomon set & \DASH{}, CPDAG \\
