...
The Lean~4 formalization (v4.30.0-rc1 with Mathlib) comprises \ClaimMainFiles~files containing \ClaimMainTheorems~theorems and lemmas, \ClaimMainAxioms~\texttt{axiom} declarations, and 0 \texttt{sorry} (unproved goals).

The axiom inventory is stratified by dependency: the core universal impossibility (\texttt{explanation\_impossibility}) requires zero axioms beyond the Rashomon hypothesis.  The \ClaimMainAxioms~\texttt{axiom} declarations (\texttt{gbdtWorld} and \texttt{gbdtAxioms}) bundle ${\sim}14$ gradient-boosting properties---model type and tree counts, split-count and first-mover structure, and measure-theoretic infrastructure---used only by the ML-specific quantitative bounds.  All former individually-declared axioms (\texttt{Model}, \texttt{firstMover\_surjective}, \texttt{proportionality\_global}, etc.) are now fields of these two bundles or theorems extracted from them, and the nine ML instances are \emph{constructive} (zero axioms; their Rashomon witnesses are checked by \texttt{decide}/\texttt{rfl}).

Key files: \texttt{ExplanationSystem.lean} (abstract impossibility), \texttt{AttributionInstanceConstructive.lean} (SHAP/IG/LIME instance), \texttt{CausalInstanceConstructive.lean} (DAG Markov equivalence), \texttt{MaximalIncompatibility.lean} (bilemma and tightness), \texttt{UniversalResolution.lean} ($G$-invariant resolution), \texttt{NecessityBiconditional.lean} (\texttt{rashomon\_biconditional\_neq}: impossibility iff Rashomon, for inequality-incompatibility; \texttt{fully\_specified\_possibility} gives the general converse), \texttt{CapacityTrace.lean} (\texttt{capacity\_eq\_trace}: capacity $=$ trace of the Reynolds projection $= \dim V^G$; \texttt{reynolds\_isIdempotent} for the actual orbit-average operator; \texttt{capacity\_le\_of\_ker}: the feature-geometry upper bound $C \leq \dim V - \dim W$).  Full file listing and build instructions are in the repository \texttt{CLAUDE.md}.
