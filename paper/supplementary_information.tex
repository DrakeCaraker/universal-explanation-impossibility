...
Mean ablation (replacing component output with its mean activation) produces $G$-invariant $\rho = 0.975$ (A) and $0.987$ (B), matching weight zeroing ($0.972$, $0.982$).  Cross-method per-model $\rho = 0.59$ (A) and $0.50$ (B): the two methods give different head-level rankings but agree on $V^G$.  The layer-level structure is method-invariant.
