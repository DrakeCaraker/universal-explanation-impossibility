...
\subsection{Instance 8: Language Model Self-Explanations}
...
\begin{itemize}
  ...
  \item \textbf{Attention, LLM explanation}: Stanford Sentiment Treebank
    (SST-2), 200 sentences.
...
\begin{itemize}
  ...
  \item \textbf{Attention / LLM}: DistilBERT (6 layers, 12 heads,
...
LLM, mechanistic interpretability, fairness).
...
\subsection{Legal Interpretation: Statutory Ambiguity}

When a statute admits multiple legally valid interpretations, each
consistent with the legislative text (the ``observable''), any judicial
explanation faces the trilemma: be faithful to the chosen interpretation,
stable across courts, and decisive in application. The impossibility
applies whenever genuinely incompatible interpretations are legally valid.
