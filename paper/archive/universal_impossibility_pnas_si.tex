...
%% Theorem environments (separate counters, matching PNAS main)
...
\subsection{Proof of Tightness (Main Text Proposition 1)}
...
\subsection{Proof of Necessity (Main Text Proposition 2)}

\begin{proof}
($\Rightarrow$) If the Rashomon property holds, the main theorem shows no
...
\begin{itemize}
  ...
  \item \textbf{Stability}: Suppose $\mathsf{observe}(\theta_1) =
    ...
    \texttt{fully\_specified\_possibility} theorem, which assumes the
    negation of the Rashomon property directly implies equality of native
    explanations on each fiber.
\end{itemize}
\end{proof}

\subsection{Proof of $G$-Invariance Implies Stability (Main Text Proposition 3)}
...
Four of twenty models achieved 72--78\% held-out accuracy; all twenty are
...
(weight perturbation): 60.0\% [95\% CI: 59--61\%]. Under full retraining
(20 models, last 2 transformer layers retrained from random
initialization), the argmax flip rate is 19.9\%. We report the
conservative retraining figure in the main text.
Mean Kendall $\tau$: 0.30.
...
equivalence. Resolution: CPDAG
reporting~\cite{chickering2002optimal}.

\paragraph{Implementation note.}
The CI extraction function \texttt{ciFromDAG} is a simplified formula
correct for chain and fork structures on three nodes. It does not
implement full $d$-separation (in particular, it does not handle
collider structures correctly). The Rashomon witness uses only chain and
fork DAGs, for which the formula is verified by computation. A complete
formalization of $d$-separation in Lean~4 is a substantial undertaking
...
\paragraph{Resolution.}
Ensemble achieves comparable performance (AUC 0.951 vs.\ 0.958
best-single) with evaluation-stable predictions across splits. Report
the Rashomon set or ensemble rather than a single ``best'' model.
...
Peak flip rate: 9.6\%. Prediction agreement: 78.8\%, reflecting sensitivity of
ImageNet-pretrained ResNet-18 to perturbation on CIFAR-10.
...
$\sH$: token citations---the top-$k$ most-attended tokens extracted via
attention rollout as a proxy for language model self-explanation.
...
We report 95\% bootstrap confidence intervals for each primary metric.
Across seven experiments with multiple metrics, the total number of
statistical comparisons is approximately 20. We do not apply formal
multiple comparison correction (e.g., Bonferroni, Benjamini--Hochberg)
for two reasons: (1)~each experiment tests a domain-specific hypothesis
that stands independently of the others, and (2)~the primary question is
not whether any single experiment reaches significance but whether the
pattern of Rashomon-driven instability is consistent across domains. All
positive results exceed the 20\% practical significance threshold by
substantial margins (range: 19.9\%--80\%), reducing the risk of false
...
The main repository has \textbf{2}~\texttt{axiom} declarations, which bundle
the ${\sim}14$ gradient-boosting properties formerly declared as separate
axioms:

\paragraph{\texttt{gbdtWorld} (1 declaration).}
Bundles the GBDT type infrastructure---model type, tree counts
(\texttt{numTrees}, \texttt{numTrees\_pos}), and the measure-theoretic
structure (\texttt{modelMeasurableSpace}, \texttt{modelMeasure}) connecting
the model space to Mathlib's measure theory library.

\paragraph{\texttt{gbdtAxioms} (1 declaration).}
Bundles the behavioral properties---\texttt{attribution}, \texttt{splitCount},
\texttt{firstMover} and their properties (first-mover surjectivity, split-count
structure, global proportionality, cross-group symmetry). These encode the
structure of gradient-boosted decision trees.

\paragraph{The nine ML instances: zero axioms.}
Formerly axiomatized per-instance type and Rashomon-witness assumptions are now
\emph{constructive}: each instance builds explicit finite witnesses whose
Rashomon property is checked by \texttt{decide}/\texttt{rfl}, introducing no
axioms (\texttt{*InstanceConstructive.lean}). All former individually-declared
axioms (\texttt{Model}, \texttt{firstMover\_surjective}, etc.) are now fields of
the two bundles above or theorems extracted from them.
...
\begin{itemize}
  ...
  \item \textbf{Attribution impossibility}
    ...
    (depends only on the \texttt{gbdtWorld} type bundle).
  \item \textbf{GBDT impossibility} (\texttt{gbdt\_impossibility\_local}),
    \textbf{quantitative bounds} (\texttt{impossibility}), and \textbf{DASH
    resolution} (\texttt{consensus\_equity}): each depends on the two bundled
    declarations \texttt{gbdtWorld} and \texttt{gbdtAxioms} (surjectivity,
    first-mover, split-count, proportionality, cross-group symmetry are fields
    of these bundles), verified by \texttt{\#print axioms}.
\end{itemize}
...
\texttt{attribution\_impossibility\_constructive} & AttributionInstanceConstructive.lean & 0 \\
\texttt{attention\_impossibility\_constructive} & AttentionInstanceConstructive.lean & 0 \\
\texttt{counterfactual\_impossibility\_constructive} & CounterfactualInstanceConstructive.lean & 0 \\
\texttt{concept\_impossibility\_constructive} & ConceptInstanceConstructive.lean & 0 \\
\texttt{causal\_impossibility\_constructive} & CausalInstanceConstructive.lean & 0 \\
\texttt{mech\_interp\_impossibility\_constructive} & MechInterpInstanceConstructive.lean & 0 \\
\texttt{gInvariant\_stable} & UniversalResolution.lean & 0 \\
\texttt{universal\_design\_space\_dichotomy} & UniversalDesignSpace.lean & 0 \\
\texttt{fully\_specified\_possibility} & Necessity.lean & 0 \\
...
\texttt{causal\_rashomon\_derived} & MarkovEquivalence.lean & 0 \\
\bottomrule
\end{longtable}
...
\begin{table}[h]
\centering
\caption{Practitioner decision table (Table~S2): which desideratum to sacrifice for
each goal.}
...
model versions implies stability. The main theorem shows all three
...
be tested in isolation. The main theorem formalizes this: the configuration
